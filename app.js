...
function loadDraft() {
  const raw = localStorage.getItem("print_draft_clean_v1");
...
function saveDraft() {
  localStorage.setItem("print_draft_clean_v1", JSON.stringify(state.draft));
}

function loadUndoStack() {
  const raw = localStorage.getItem("print_undo_stack_clean_v1");
  if (!raw) return [];
  try {
    const parsed = JSON.parse(raw);
...
function saveUndoStack() {
  localStorage.setItem("print_undo_stack_clean_v1", JSON.stringify(state.undoStack.slice(-25)));
}
...
function normalizeColorEntry(entry) {
  ...
  return {
    key: entry.key || "color",
    slotLabel: entry.slotLabel || entry.label || "Farbe",
    colorName: entry.colorName || entry.value || entry.name || "",
    hex: normalizeHex(entry.hex || "#d4af37")
  };
}

function getColorButtonDisplayText(color) {
  const slot = color.slotLabel || "Farbe";
  const chosen = (color.colorName || "").trim();
  return chosen ? `${slot} · ${chosen}` : slot;
}

function getColorPreviewText(slotLabel, colorName) {
  return `${slotLabel || "Farbe"} · ${colorName || "ausgewählt"}`;
}
...
function setActiveColorChoice(buttonEl) {
  ...
  if (buttonEl) buttonEl.classList.add("active-color-choice");
  state.selectedColorSourceBtn = buttonEl || null;
}
...
function getCurrentProductConfig() {
  ...
  if (product === "lumi") {
    return {
      sizeRequired: false,
      colors: [{ key: "body", slotLabel: "Farbe", colorName: "", hex: "#d4af37" }],
    ...
    return `
      ...
      >
        <span class="color-chip-label">
          ...
          <span class="color-text">${escapeHtml(getColorButtonDisplayText(color))}</span>
        </span>
        <span>Ändern</span>
      </button>
    `;
...
function updateColorPreviewText() {
  els.colorPreviewText.textContent = getColorPreviewText(
    els.colorNameInput.value.trim() || "Farbe",
    els.favoriteColorNameInput.value.trim() || "ausgewählt"
  );
}
...
function collectColorData() {
  ...
  return buttons.map((btn) => normalizeColorEntry({
...
function collectFormValues(applyValidation = true) {
  ...
  const hasValidColors =
    ...
    colors.every((color) => {
      ...
      return normalized.slotLabel.trim() !== "";
    });
...
function renderColorListText(colors) {
  ...
  return colors
    .map((rawColor) => {
      const c = normalizeColorEntry(rawColor);
      const shownValue = c.colorName || "ausgewählt";
      return `${escapeHtml(c.slotLabel)}: ${escapeHtml(shownValue)}`;
    })
    .join(", ");
}
...
function resetForm(options = {}) {
  ...
  const currentCustomerName = preserveCustomerName ? els.customerName.value : "";
  document.getElementById("itemForm").reset();
  setActiveChoice(els.priorityChoices, "Wichtig");
  renderProductForm();
  if (preserveCustomerName) {
    els.customerName.value = currentCustomerName;
  }
}

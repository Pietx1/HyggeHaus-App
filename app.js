...
function loadDraft() {
  const raw = localStorage.getItem("print_draft_v7");
...
function saveDraft() {
  localStorage.setItem("print_draft_v7", JSON.stringify(state.draft));
}

function loadUndoStack() {
  const raw = localStorage.getItem("print_undo_stack_v7");
  if (!raw) return [];
  try {
    return JSON.parse(raw);
  } catch {
...
function saveUndoStack() {
  localStorage.setItem("print_undo_stack_v7", JSON.stringify(state.undoStack.slice(-25)));
}
  ...
  if (rabbitColorModeChoices) {
    rabbitColorModeChoices.querySelectorAll("button").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        const colorsToRender = btn.dataset.value === "single" ? config.colorsSingle : config.colorsMulti;
...
function renderColorFields(colors) {
  ...
  wrap.innerHTML = colors.map((rawColor) => {
    ...
    const displayText = getColorButtonDisplayText(color);
    ...
    return `
      ...
      >
        <span class="color-chip-label">
          ...
          <span class="color-text">${escapeHtml(displayText)}</span>
        </span>
        <span>Ändern</span>
      </button>
    `;
...
function collectFormValues(applyValidation = true) {
  ...
  const hasValidColors =
    ...
    colors.length > 0 &&
    colors.every((c) => {
      const normalized = normalizeColorEntry(c);
...
function renderDraft() {
  ...
  els.draftItems.innerHTML = items.map((item) => {
    const colors = renderColorListText(item.colors);
...
function resetForm(options = {}) {
  const { preserveCustomerName = false } = options;

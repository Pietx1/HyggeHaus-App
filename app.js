...
function loadDraft() {
  const raw = localStorage.getItem("print_draft_v6");
...
function saveDraft() {
  localStorage.setItem("print_draft_v6", JSON.stringify(state.draft));
}

function loadUndoStack() {
  const raw = localStorage.getItem("print_undo_stack_v6");
  if (!raw) return [];
  try {
    return JSON.parse(raw);
  } catch {
...
function saveUndoStack() {
  localStorage.setItem("print_undo_stack_v6", JSON.stringify(state.undoStack.slice(-25)));
}
    ...
    return `
      ...
      >
        <span class="color-chip-label">
          ...
          <span class="color-text">${escapeHtml(color.slotLabel)}</span>
        </span>
        <span>Ändern</span>
      </button>
    `;
...
function handleAddExtraColor() {
  ...
  btn.dataset.slotLabel = `Farbe ${newIndex}`;

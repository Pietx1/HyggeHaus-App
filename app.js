...
const els = {
  ...
  priceKeypad: document.getElementById("priceKeypad"),
...
function loadDraft() {
  const raw = localStorage.getItem("print_draft_final_clean");
  if (!raw) return { mode: "order", items: [] };
  try {
    const parsed = JSON.parse(raw);
    return {
      mode: parsed.mode || "order",
...
function saveDraft() {
  localStorage.setItem("print_draft_final_clean", JSON.stringify(state.draft));
}

function loadUndoStack() {
  const raw = localStorage.getItem("print_undo_stack_final_clean");
  if (!raw) return [];
  try {
    const parsed = JSON.parse(raw);
...
function saveUndoStack() {
  localStorage.setItem("print_undo_stack_final_clean", JSON.stringify(state.undoStack.slice(-25)));
}
...
function normalizeColorEntry(entry) {
  return {
    key: entry?.key || "color",
    slotLabel: entry?.slotLabel || "Farbe",
    colorName: entry?.colorName || "",
    hex: normalizeHex(entry?.hex || "#d4af37")
  };
}

function colorLine(color) {
  const c = normalizeColorEntry(color);
  return `${c.slotLabel}: ${c.colorName || "nicht gesetzt"}`;
}

function colorButtonText(color) {
  const c = normalizeColorEntry(color);
  return c.colorName ? `${c.slotLabel} · ${c.colorName}` : c.slotLabel;
}
...
function getCurrentProductConfig() {
  ...
  if (product === "rabbit") {
    return {
      ...
      colorsMulti: [
        { key: "ears", slotLabel: "Ohren", colorName: "", hex: "#d4af37" },
        { key: "body", slotLabel: "Körper", colorName: "", hex: "#d4af37" },
        { key: "tail", slotLabel: "Popo", colorName: "", hex: "#d4af37" }
      ],
      colorsSingle: [
        { key: "main", slotLabel: "Farbe", colorName: "", hex: "#d4af37" }
      ],
      verticalColors: true,
  ...
  if (product === "lumi") {
    return {
      sizeRequired: false,
      colors: [
        { key: "body", slotLabel: "Farbe", colorName: "", hex: "#d4af37" }
      ],
  ...
  if (product === "text") {
    return {
      ...
      sizeLabel: "Größe",
      colors: [
        { key: "c1", slotLabel: "Erste Farbe", colorName: "", hex: "#d4af37" },
        { key: "c2", slotLabel: "Zweite Farbe", colorName: "", hex: "#d4af37" }
      ],
...
function renderProductForm() {
  ...
  if (config.rabbitColorModeEnabled) initialColors = config.colorsMulti;
  renderColorFields(initialColors);

  const sizeChoices = document.getElementById("sizeChoices");
  if (sizeChoices) {
    sizeChoices.querySelectorAll(".size-btn").forEach((btn) => {
  ...
  if (rabbitColorModeChoices) {
    rabbitColorModeChoices.querySelectorAll(".choice-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        setActiveChoice(rabbitColorModeChoices, btn.dataset.value);
        renderColorFields(btn.dataset.value === "single" ? config.colorsSingle : config.colorsMulti);
      });
    });
  }
  ...
  if (quantityChoices) {
    quantityChoices.querySelectorAll(".quantity-btn").forEach((btn) => {
  ...
  if (manualPriceBtn) {
    manualPriceBtn.addEventListener("click", () => {
      openPriceDialog("form-item", document.getElementById("manualPriceRaw")?.value || "", "Preis eingeben");
    });
  }
}
...
function renderColorFields(colors) {
  ...
  wrap.innerHTML = colors.map((rawColor) => {
    const color = normalizeColorEntry(rawColor);
    return `
      <button
        type="button"
        class="color-chip-btn"
        data-color-key="${color.key}"
        data-slot-label="${escapeHtml(color.slotLabel)}"
        data-color-name="${escapeHtml(color.colorName)}"
        data-color-hex="${color.hex}"
      >
        <span class="color-chip-label">
          <span class="color-dot" style="background:${color.hex};"></span>
          <span class="color-text">${escapeHtml(colorButtonText(color))}</span>
        </span>
        <span>Ändern</span>
      </button>
    `;
...
function handleAddExtraColor() {
  ...
  const color = {
    key: `c${currentCount + 1}`,
    slotLabel: `Farbe ${currentCount + 1}`,
    colorName: "",
    hex: "#d4af37"
  };
  ...
  btn.dataset.colorKey = color.key;
  btn.dataset.slotLabel = color.slotLabel;
...
function updateColorPreview() {
  ...
  const colorName = els.favoriteColorNameInput.value.trim() || "nicht gesetzt";
  els.colorPreviewText.textContent = `${slotLabel}: ${colorName}`;
}

function openColorModal(targetBtn) {
  state.colorModalTarget = targetBtn;
  state.selectedFavoriteId = null;
  setActiveColorChoice(null);

  const slotLabel = targetBtn.dataset.slotLabel || "Farbe";
...
async function renderColorButtons() {
  els.presetColors.innerHTML = PRESET_COLORS.map(
    (c) => `
      <button type="button" class="preset-color-btn" data-name="${escapeHtml(c.name)}" data-hex="${c.hex}" data-kind="preset">
  ...
  els.favoriteColors.innerHTML = favorites.length
    ? favorites.map(
        (c) => `
          <button type="button" class="preset-color-btn" data-id="${c.id}" data-name="${escapeHtml(c.name)}" data-hex="${c.hex}" data-kind="favorite">
            <span class="color-dot" style="background:${c.hex}"></span>
            <span>${escapeHtml(c.name)}</span>
          </button>
        `
  ...
  [...els.presetColors.querySelectorAll(".preset-color-btn"), ...els.favoriteColors.querySelectorAll(".preset-color-btn")].forEach((btn) => {
    btn.addEventListener("click", () => {
      els.customColorPicker.value = btn.dataset.hex;
      els.liveColorPreview.style.background = btn.dataset.hex;
      els.favoriteColorNameInput.value = btn.dataset.name || "";
      updateColorPreview();

      if (btn.dataset.kind === "favorite") {
        state.selectedFavoriteId = btn.dataset.id;
...
function collectColorData() {
  ...
  return buttons.map((btn) =>
    normalizeColorEntry({
      ...
      slotLabel: btn.dataset.slotLabel,
...
function collectFormValues(applyValidation = true) {
  ...
  const hasValidColors =
    ...
    colors.length > 0 &&
    colors.every((c) => normalizeColorEntry(c).slotLabel.trim() !== "");
...
function renderColorListText(colors) {
  ...
  return colors.map((c) => colorLine(c)).join(", ");
}
  ...
  els.draftItems.innerHTML = items.map((item) => `
    <div class="draft-card">
      ...
      <div class="item-meta">
        ...
        ${item.productType === "rabbit" ? `Farbmodus: ${item.rabbitColorMode === "single" ? "Einfarbig" : "Mehrfarbig"}<br>` : ""}
        ${renderColorListText(item.colors)}<br>
...
function resetForm(options = {}) {
  const preserveCustomerName = Boolean(options.preserveCustomerName);
  const storedCustomerName = preserveCustomerName ? els.customerName.value : "";
  document.getElementById("itemForm").reset();
  setActiveChoice(els.priorityChoices, "Wichtig");
  renderProductForm();
  if (preserveCustomerName) {
    els.customerName.value = storedCustomerName;
  }
}
...
async function addItemToDraft() {
  try {
    ...
    saveDraft();
    renderDraft();
    resetForm({ preserveCustomerName: true });
  } catch (error) {
      ...
      <div class="order-card-top">
        ...
        ${!archived ? `
          <div class="order-actions">
            <button class="small-btn success archive-btn" data-order-id="${order.id}" type="button">Archivieren</button>
            <button class="small-btn danger delete-order-btn" data-order-id="${order.id}" type="button">Löschen</button>
          </div>
        ` : `
          <div class="order-actions">
            <button class="small-btn danger delete-order-btn" data-order-id="${order.id}" type="button">Löschen</button>
  ...
  return `
    <div class="stock-entry" data-order-id="${order.id}">
      <div class="stock-entry-main">
        ...
        Menge: ${item.quantity}<br>
        ${item.size_text ? `Größe: ${escapeHtml(item.size_text)}<br>` : ""}
        ${renderColorListText(item.colors)}
      </div>

      <div>
        <button class="small-btn danger delete-stock-btn" data-order-id="${order.id}" type="button">Löschen</button>
      </div>
    </div>
  `;
}
  ...
  document.querySelectorAll(".item-done-checkbox").forEach((checkbox) => {
    checkbox.addEventListener("change", async () => {
      ...
      pushUndo({
        ...
        label: `Position ${newValue ? "als erledigt markieren" : "als offen markieren"}`,
        itemId,
        orderId,
...
function bindEvents() {
  ...
  els.productTabs.querySelectorAll(".product-tab").forEach((btn) => {
    btn.addEventListener("click", () => switchProduct(btn.dataset.product));
  });

  els.priorityChoices.querySelectorAll(".choice-btn").forEach((btn) => {
  ...
  els.applyColorBtn.addEventListener("click", () => {
    ...
    const color = normalizeColorEntry({
      key: state.colorModalTarget.dataset.colorKey,
      slotLabel,
      colorName,
      hex
    });
  ...
  els.priceKeypad.querySelectorAll(".price-key").forEach((btn) => {
